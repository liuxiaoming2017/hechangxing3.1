...
- (UIImageView *)messageVoiceImage{
    if (!_messageVoiceImage) {
        ...
        self.messageVoiceImage.image = [self imageNamed:@"voice_play_animation_0.png"];
...
- (void)setHudType:(NSInteger)hudType{
    ...
    switch (hudType) {
        ...
        case 3://警告
        {
            ...
            self.messageVoiceImage.image = [self imageNamed:@"voice_failure.png"];
            [self.overlayWindow addSubview:self];
...
- (void)showVoiceMeters:(int)meter {
    ...
    if (self.hudType == 1) {
        ...
        self.messageVoiceImage.image = [self imageNamed:imageName];
    }
}
