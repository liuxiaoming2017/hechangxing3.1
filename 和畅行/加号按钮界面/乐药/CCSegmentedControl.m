...
- (void)insertSegmentWithTitle:(NSString *)title atIndex:(NSUInteger)segmentIndex animated:(BOOL)animated
{
    ...
    segmentView.textColor = [UIColor whiteColor];//self.segmentTextColor;
    ...
    else
    {
        ...
        segmentView.center = CGPointMake(self.center.x,self.center.y);//self.center;
...
- (void)layoutSubviews
{
    ...
    [self.items enumerateObjectsUsingBlock:^(UIView *item, NSUInteger idx, BOOL *stop)
     {
         ...
         {
             [item sizeToFit];
             item.center = CGPointMake(pos + CGRectGetWidth(item.bounds) / 2, itemsVAlignCenter-3);
         }
         else
         {
             item.frame = CGRectMake(pos, 0, CGRectGetWidth(item.bounds), itemsVAlignCenter * 2-6);
         }
    ...
    else
    {
        UIView *selectedItem = self.items[self.selectedSegmentIndex];
         UIImage* selestainbg=[UIImage imageNamed:@"YY_btn_bg.png"];
        
        CGRect stainFrame = CGRectInset(selectedItem.frame, -4, -10);
//        self.selectedStainView.layer.cornerRadius = stainFrame.size.height / 2;
        ...
        [UIView animateWithDuration:animated ? 0.2 : 0 animations:^
         {
             self.selectedStainView.frame = CGRectMake(stainFrame.origin.x-4, stainFrame.origin.y+3, selestainbg.size.width/2, selestainbg.size.height/2);//stainFrame;
         }
                         completion:^(BOOL finished)
         {
             for (UILabel *item in self.items)
             {
                // item.textAlignment = NSTextAlignmentCenter;
                 ...
                 {
                     item.textColor = [UtilityFunc colorWithHexString:@"#2fa1fb"];//self.selectedSegmentTextColor;
                 }
                 else
                 {
                     item.textColor =[UIColor whiteColor] ;//self.segmentTextColor;
                 }
             }
...
{
    _selectedStainView.frame=selectedStainView.frame;
