...
float pixelsPerCell = 32.00; // 0.2 second per cell

float lineWidth_Grid = 0.5;
float lineWidth_LiveMonitor = 1.3;
float lineWidth_Static = 1;

int pointsPerSecond = 250;
float pixelPerPoint = 2 * 60.0f / 250.0f;
int pointPerDraw = 250.0f * 0.04f;
...
- (void)fireDrawing
{
	...
	for (int i=0; i<pointCount; i++)
    {
		if ([self pointAvailable:currentPoint])
		{
            ...
            int num = [[self.pointsArray objectAtIndex:currentPoint] intValue];
			CGFloat value = full_height * 0.9 - num * uVpb * pixelPerUV + 13;
//            NSLog(@"pos_x : %f ||  num %d -------- value : %f",pos_x,num,value); 
...
{
	NSInteger pCount = self.pointsArray.count;
