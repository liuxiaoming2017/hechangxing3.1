...
- (void)viewDidLoad {
    ...
    //self.blurView.backgroundColor = [UIColor orangeColor];
...
#pragma mark - 显示/隐藏
- (BOOL)isSidebarShown{
    return self.contentView.frame.origin.x < ScreenWidth ? YES :NO;
}

- (void)showHideSidebar
{
    if (self.contentView.frame.origin.x == ScreenWidth) {
        LLLog(@"开始时在-点");
...
- (void)autoShowHideSidebar
{
    ...
    else
    {
        LLLog(@"自动缩回");
        [UIView animateWithDuration:duration animations:^{
            [self setSidebarOriginX:ScreenWidth];
        } completion:^(BOOL finished) {
            _isMoving = NO;
            self.view.hidden = YES;
        }];
...
- (void)beginShowSidebar
{
    ...
    if (self.contentView.frame.origin.x == ScreenWidth) { //self.blurView.alpha < 0.5 &&
        
        // 截图
        //self.snapImageView.image = [self imageFromView:self.view.superview];
        self.snapImageView.image = [UIImage createImageWithColor:UIColorFromHex(0X0E0E0E)];
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
    ...
    if (point.x > ScreenWidth - kSidebarWidth) {
        return NO;
    }
    return  YES;
}
...
- (void)setBlurViewAlpha
{
    ...
    float percent = (ScreenWidth - rect.origin.x) / kSidebarWidth;
