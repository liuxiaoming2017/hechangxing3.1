...
- (void)initWithProperty
{
    self.itemMargin = 20;
    self.selectIndex = 0;
    self.titleNormalColor = RGB_TextGray;
...
- (void)setTitlesArr:(NSArray *)titlesArr
{
    ...
    for (NSString *title in titlesArr) {
        ...
        if(self.itemBtnArr.count == 0){
            [btn setBackgroundColor:RGB_TextAppBlue];
        }
...
- (void)setSelectIndex:(NSInteger)selectIndex
{
    ...
    lastBtn.selected = NO;
    lastBtn.backgroundColor =RGB_AppWhite;
    
    _selectIndex = selectIndex;
    UIButton *currentBtn = [self.scrollView viewWithTag:_selectIndex + 666];
    currentBtn.selected = YES;
   currentBtn .backgroundColor =RGB_TextAppBlue;
    [self setNeedsLayout];
    [self layoutIfNeeded];
}
