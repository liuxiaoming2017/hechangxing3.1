...
-(void)setIsWaveing:(BOOL)flag{
    [_waterWaveView setIsWaveing:flag];
}

-(BOOL)getIsWaveing{
    return [_waterWaveView getIsWaveing];
}
...
-(void)setNeedStop:(BOOL)needStop{
    _waterWaveView.needStop = needStop;
}

-(BOOL)getNeedStop{
    return _waterWaveView.needStop;
}

-(BOOL)getIsHeighest{
    return [_waterWaveView getIsHeighest];
}

- (void)dealloc{
    NSLog(@"---TYWaveProgressView---dealloc---");
    // 很重要 否则wave不能释放
    [self stopWave];
}
