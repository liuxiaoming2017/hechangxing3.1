...
NSString const *CWTouchSizeDisappear = @"CWTouchSizeDisappear";
NSString const *CWKeyBoardlsVisible = @"CWKeyBoardlsVisible";

@implementation UIViewController (CWPopup)

@dynamic popupViewController, useBlurForPopup,keyBoardlsVisible;
...
- (void)presentPopupViewControllerwithRect:(UIViewController *)viewControllerToPresent Posation:(CGRect)rect animated:(BOOL)flag completion:(void (^)(void))completion {
    if (self.popupViewController == nil) {
        // initial setup
        self.popupViewController = viewControllerToPresent;
...
//       CGRect finalFrame = [self getPopupFrameForViewController:viewControllerToPresent];
        ...
        frame.size.width = finalFrame.size.width;
        frame.size.height = finalFrame.size.height;
...
- (void)presentPopupViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag dismiss:(BOOL)dis completion:(void (^)(void))completion {
    if (self.popupViewController == nil) {
        // initial setup
        self.popupViewController = viewControllerToPresent;
        self.popupViewController.view.autoresizesSubviews = NO;
...
- (void)presentPopupPickerVC:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)(void))completion {
    if (self.popupViewController == nil) {
        // initial setup
        self.popupViewController = viewControllerToPresent;
        self.popupViewController.view.autoresizesSubviews = NO;
        self.popupViewController.view.autoresizingMask = UIViewAutoresizingNone;
        [self.popupViewController viewWillAppear:YES];
        CGRect finalFrame = [self getPickerFrameForViewController:viewControllerToPresent];
        // parallax setup if iOS7+
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0) {
            UIInterpolatingMotionEffect *interpolationHorizontal = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.x" type:UIInterpolatingMotionEffectTypeTiltAlongHorizontalAxis];
        ...
        if (self.useBlurForPopup) {
            ...
            CGRect rect = CGRectMake(0, 0, SCREEN_WID, SCREEN_HEI);
...
//    NSLog(@"键盘弹出 - %d",self.keyBoardlsVisible);
}
//  键盘隐藏触发该方法
- (void)keyboardDidHide
{
    self.keyBoardlsVisible =NO;
//    NSLog(@"键盘隐藏 - %d",self.keyBoardlsVisible);
}
- (void)event:(UITapGestureRecognizer *)gesture
{
    if (self.keyBoardlsVisible) {
        [self.popupViewController.view endEditing:YES];
    }else{
...
- (CGRect)getPickerFrameForViewController:(UIViewController *)viewController {
    CGRect frame = viewController.view.frame;
    CGFloat x;
    CGFloat y;
    if (UIDeviceOrientationIsPortrait((UIDeviceOrientation)[UIApplication sharedApplication].statusBarOrientation)) {
...
- (void)setKeyBoardlsVisible:(BOOL)keyBoardlsVisible{
    if ([[[UIDevice currentDevice] systemVersion] floatValue] < 7.0 && keyBoardlsVisible) {
        NSLog(@"ERROR: Blur unavailable prior to iOS 7");
        objc_setAssociatedObject(self, &CWKeyBoardlsVisible, [NSNumber numberWithBool:NO], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    } else {
        objc_setAssociatedObject(self, &CWKeyBoardlsVisible, [NSNumber numberWithBool:keyBoardlsVisible], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
}

-(BOOL)keyBoardlsVisible{
    NSNumber *result = objc_getAssociatedObject(self, &CWKeyBoardlsVisible);
    return [result boolValue];
}
